...
export async function callGenerativeAI(initialContents, tools // Still expect Vertex Tool format initially
) {
    const { provider, modelId, temperature, useStreaming, maxOutputTokens, maxRetries, retryDelayMs, } = aiConfig;
    const isGroundingRequested = tools?.some(tool => tool.googleSearchRetrieval);
    let filteredToolsForVertex = tools;
    let adaptedToolsForGemini = undefined;
    if (provider === 'gemini' && tools) {
        const nonSearchTools = tools.filter(tool => !tool.googleSearchRetrieval);
        ...
        filteredToolsForVertex = tools.filter(tool => tool.googleSearchRetrieval);
    }
    // Get appropriate model instance
    let vertexModelInstance;
    let geminiModelInstance;
    if (provider === 'vertex') {
        vertexModelInstance = isGroundingRequested
            ? generativeClient.preview.getGenerativeModel({ model: modelId })
            : generativeClient.getGenerativeModel({ model: modelId });
    }
    else { // gemini
        geminiModelInstance = generativeClient.getGenerativeModel({
            model: modelId,
    ...
    const resolvedVertexSafetySettings = vertexSafetySettings;
    const resolvedGeminiSafetySettings = configGeminiSafetySettings;
    ...
    const geminiGenConfig = commonGenConfig;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (useStreaming) {
                let accumulatedText = "";
                let finalAggregatedResponse;
                if (provider === 'vertex') {
                    if (!vertexModelInstance)
                        throw new Error("Vertex model instance not initialized.");
                    ...
                    if (!geminiModelInstance)
                        throw new Error("Gemini model instance not initialized.");
                ...
                if (!accumulatedText && finalAggregatedResponse) {
                    try {
                        if (provider === 'vertex') {
                            ...
                            if (typeof aggregatedTextVertex === 'string')
                                accumulatedText = aggregatedTextVertex;
                        }
                        else { // gemini
                            const aggregatedTextGemini = finalAggregatedResponse.text();
                            if (typeof aggregatedTextGemini === 'string')
                                accumulatedText = aggregatedTextGemini;
                        }
                    }
                    catch (e) {
                ...
                if (provider === 'vertex') {
                    ...
                    try {
                        ...
                        if (e.message?.toLowerCase().includes('safety') || e.message?.toLowerCase().includes('prompt blocked') || e.status === 'BLOCKED') {
            ...
            const isRetryable = !isBlockingError && (errorMessageString.includes('429') ||
